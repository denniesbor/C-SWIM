...
def plot_socio_economic_impact(results_data, confidence_df, model_type="io"):
    ...
    if USE_ALPHA_BETA_SCENARIO:
        scenario_map = {"gic_75yr": 75, "gic_100yr": 100, "gic_125yr": 125, "gic_150yr": 150, 
                        "gic_175yr": 175, "gic_200yr": 200, "gic_225yr": 225, "gic_250yr": 250}
    else:
        scenario_map = {"50-year": 50, '75-year': 75, '100-year': 100, '125-year': 125, '150-year': 150, 
                        '175-year': 175, '200-year': 200, '225-year': 225, '250-year': 250}
    ...
    est_data = conf_filtered[conf_filtered['variable'] == 'EST_TOTAL'].set_index('scenario')
    est_x = [scenario_map[s.replace('e_', '').replace('-hazard A/ph', '')] for s in est_data.index]
    ...
    est_x_sorted, est_mean_sorted, est_err_low_sorted, est_err_high_sorted = zip(*est_sorted)

    pop_data = conf_filtered[conf_filtered['variable'] == 'POP_AFFECTED'].set_index('scenario')
    pop_x = [scenario_map[s.replace('e_', '').replace('-hazard A/ph', '')] for s in pop_data.index]
    ...
    pop_x_sorted, pop_mean_sorted, pop_err_low_sorted, pop_err_high_sorted = zip(*pop_sorted)

    direct_summary = econ_filtered.groupby(['scenario', 'confidence'])['direct_shock'].sum().abs().unstack()
    ...
    direct_x_sorted, direct_mean_sorted, direct_err_low_sorted, direct_err_high_sorted = zip(*direct_sorted)

    total_summary = econ_filtered.groupby(['scenario', 'confidence'])['total_impact'].sum().abs().unstack()
    ...
    total_x_sorted, total_mean_sorted, total_err_low_sorted, total_err_high_sorted = zip(*total_sorted)
    ...
    axes[0,0].errorbar(est_x_sorted, est_mean_sorted, yerr=[est_err_low_sorted, est_err_high_sorted],
                    fmt='o', capsize=4, capthick=2, color='green')
    ...
    axes[0,1].errorbar(pop_x_sorted, pop_mean_sorted, yerr=[pop_err_low_sorted, pop_err_high_sorted],
                    fmt='o', capsize=4, capthick=2, color='red')
    ...
    axes[1,0].errorbar(direct_x_sorted, direct_mean_sorted, yerr=[direct_err_low_sorted, direct_err_high_sorted],
                    fmt='o', capsize=4, capthick=2, color='purple')
    ...
    axes[1,1].errorbar(total_x_sorted, total_mean_sorted, yerr=[total_err_low_sorted, total_err_high_sorted],
                    fmt='o', capsize=4, capthick=2, color='blue')
    ...
    fig.savefig(FIGURES_DIR / f"economic_impact_{model_type}.pdf", dpi=300, bbox_inches="tight")
    fig.savefig(FIGURES_DIR / f"economic_impact_{model_type}.png", dpi=300, bbox_inches="tight")
    plt.show()
...
def plot_vuln_trafos(vuln_data, df_lines):
    ...
    def setup_map(ax, spatial_extent=[-120, -75, 25, 50]):
        ...
        gl = ax.gridlines(draw_labels=False, linewidth=0.2, color="grey", alpha=0.5, linestyle="--")
        return ax
    ...
    if USE_ALPHA_BETA_SCENARIO:
        scenarios_to_plot = [
            'gic_100yr_mean_prediction', 'gic_150yr_mean_prediction', 
            'gic_200yr_mean_prediction', 'gic_250yr_mean_prediction'
        ]
    else:
        scenarios_to_plot = ['e_100-year-hazard A/ph', 'e_150-year-hazard A/ph', 
                            'e_200-year-hazard A/ph', 'e_250-year-hazard A/ph']
    ...
    for i, scenario in enumerate(scenarios_to_plot):
        ...
        if i == 0:
            ax.text(0, 1.2, 'Substations Exceeding 50% Failure Probability', 
                   transform=ax.transAxes, fontsize=12, fontweight='bold', va='center')
        
        scenario_vulnerable = vuln_data[
            (vuln_data['scenario'] == scenario) & 
            (vuln_data['mean_failure_prob'] > 0.4)
        ]['sub_id'].unique()
        ...
        ax.scatter(non_vulnerable['longitude'], non_vulnerable['latitude'], 
                c='black', s=0.5, alpha=0.4, transform=ccrs.PlateCarree(), zorder=3)
        
        vulnerable_coords = all_coords[all_coords.index.isin(scenario_vulnerable)]
        ax.scatter(vulnerable_coords['longitude'], vulnerable_coords['latitude'], 
                s=100, facecolors='red', edgecolors='darkred', alpha=0.6,
                linewidths=1, transform=ccrs.PlateCarree(), zorder=4)
        ...
        lc = LineCollection(line_coords, linewidths=line_widths, alpha=0.8, 
                        colors=line_colors, transform=ccrs.PlateCarree(), zorder=5)
        ax.add_collection(lc)
        
        scenario_name = scenario.replace('e_', '').replace('-hazard A/ph', '')
        ax.set_title(f'({chr(97 + i)}) {scenario_name} Storm', 
                    fontsize=10, loc="left")

    legend_elements = []
    for voltage in voltage_levels:
        legend_elements.append(
            Line2D([0], [0], color=voltage_color_map[voltage], linewidth=voltage_width_map[voltage], 
                label=f'{voltage} kV', alpha=0.6)
        )
    legend_elements.append(Line2D([0], [0], marker='o', color='w', markerfacecolor='black', markersize=4, 
            alpha=0.4, label='Other Substations'))

    legend = fig.legend(handles=legend_elements, loc='lower center', frameon=False, 
                        fontsize=9, ncol=6, bbox_to_anchor=(0.5, -0.04))
...
def plot_econo_naics(econ_results, model_type="io"):
    ...
    scenario_250_mean = econ_results[(econ_results.scenario == '250-year') &
                                (econ_results.confidence == 'mean')].set_index('sector')
    sector_order = scenario_250_mean.total_impact.abs().sort_values(ascending=False).index
    ...
    scenario_250_p95 = econ_results[(econ_results.scenario == '250-year') &
                                (econ_results.confidence == 'p95')].set_index('sector')
    ...
    for i, scen in enumerate(scenarios):
        ...
        ax.barh(y, direct,
            left=baseline_gap,
            color='darkred', alpha=.8, height=.8, zorder=3)
        ax.barh(y, multiplier,
            left=direct + baseline_gap,
            color='lightcoral', alpha=.8, height=.8, zorder=3)
        
        ax.errorbar(mean_tot + baseline_gap, y,
                    xerr=[lower_err, upper_err],
                    fmt='none', color='black',
                    capsize=2, capthick=1, elinewidth=1, zorder=4)
        ...
        sx, sy, dy = .6, .5, .07
        ax.text(sx, sy, scen, transform=ax.transAxes,
        ...
        ax.add_patch(Rectangle((sx, sy-4.7*dy), 0.04, 0.03, 
                            facecolor='darkred', alpha=0.8, transform=ax.transAxes))
        ax.text(sx+0.05, sy-4.7*dy+0.015, 'Direct Impact', transform=ax.transAxes, 
    ...
    fig.savefig(FIGURES_DIR / f"indirect_impact_{model_type}.pdf", dpi=300, bbox_inches="tight")
    fig.savefig(FIGURES_DIR / f"indirect_impact_{model_type}.png", dpi=300, bbox_inches="tight")
    plt.show()