...
from configs import (
    setup_logger, get_data_dir,
    USE_ALPHA_BETA_SCENARIO, get_scenarios,
    GDP_COLUMNS, EST_COLUMNS, DROP_COLUMNS, CSV_DTYPES,
    get_simulation_config, ALPHA_BETA_GIC_FILE, GIC_DIRECTORIES, OUTPUT_FILES
)
...
GDP_COLS = GDP_COLUMNS  # Keep backward compatibility with existing variable names
...
def one_draw_vec(batch, n_tr, pop, gic_ref, gdp_df, est_df, theta0=75.0):
    ...
    if USE_ALPHA_BETA_SCENARIO:
        Cpen = 1.0  # No penalty - trust the uncertainty-quantified predictions
    else:
        Cpen = _rng.uniform(0.60, 1.00, (batch, 1))  # Original overprediction penalty
    age    = np.where(_rng.random((batch, n_tr)) < .55,
...
def _simulate_scenario(scenario, merged, theta0, tol, max_iter, batch):
    ...
    while i < max_iter:
        ...
        p, g, e, f = one_draw_vec(
            b, n_tr, pop, gic, v[GDP_COLS], v[EST_COLS], theta0
        )
        sl = slice(i, i + b)
        pop_s[sl] = p
        for c in GDP_COLS: gdp_s[c][sl] = g[c]
        for c in EST_COLS: est_s[c][sl] = e[c]
        fail_c += f.sum(axis=0)
        i += b
    ...
    for c in GDP_COLS: res[f'{c}_affected'] = gdp_s[c][:i].mean()
    for c in EST_COLS: res[f'{c}_affected'] = est_s[c][:i].mean()
...
def process_gic_file(df: pd.DataFrame) -> pd.DataFrame:
    ...
    coords  = df.groupby('sub_id', observed=True)[['latitude', 'longitude']].first().reset_index()
...
def build_merged(agg_df, gic_df):
    return agg_df.merge(
        gic_df[['sub_id'] + SCENARIOS + ['latitude', 'longitude']],
        on='sub_id'
    )
...
def process_all_files(gic_files, aggregate_gdf, theta0=None, tol=None,
                      max_iter=None, batch=None, workers=os.cpu_count(), save_batch=None):
    ...
    # Check what batches already exist (for restart capability)
    ...
    if start_batch > 0:
        logger.info(f"Found {start_batch} existing batches, resuming from batch {start_batch}")
    
    # metric list from first file
    ...
    for batch_idx in range(start_batch, total_batches):
        ...
        logger.info(f"Processing batch {batch_idx+1}/{total_batches}: files {start_idx}-{end_idx}")
        ...
        with ProcessPoolExecutor(max_workers=workers) as ex:
            for ds, vt in tqdm(ex.map(_worker, args), total=len(batch_files), 
        ...
        batch_vuln.to_pandas().to_parquet(batch_dir / f"vuln_batch_{batch_idx:03d}.parquet")
    ...
    try:
        ...
        # Close all datasets to free up file handles
        for ds in datasets:
            ds.close()

    # Combine vulnerability datasets with better resource management
    vuln_files = sorted(batch_dir.glob("vuln_batch_*.parquet"))
    
    # Clear existing vulnerable substations directory to avoid conflicts
    vuln_output_path = DATA_LOC / OUTPUT_FILES['vulnerable_regular']
    if vuln_output_path.exists():
        import shutil
        shutil.rmtree(vuln_output_path)
    
    # Process files in smaller chunks to avoid too many open files
    ...
    if all_vuln_dfs:
        ...
        # Write with reduced partition depth to avoid too many files
        vuln_table = pa.Table.from_pandas(combined_vuln)
        pq.write_to_dataset(
            vuln_table,
            root_path=vuln_output_path,
            partition_cols=['scenario'],  # Only partition by scenario, not realization
            compression='snappy',
...
def main():
    ...
    if USE_ALPHA_BETA_SCENARIO:
        ...
        # Convert to regular DataFrame and select required columns
        gic_df = pd.DataFrame(gic_file)
        required_cols = ['sub_id', 'latitude', 'longitude'] + SCENARIOS
        gic_df = gic_df[required_cols]
        
        # Take absolute values of GIC columns
        gic_df[SCENARIOS] = gic_df[SCENARIOS].abs()
        
        # Merge with aggregate data
        merged = build_merged(aggregate_gdf, gic_df)
        
        # Run reliability engineering model for each scenario
        scen_res, vuln_tbls = {}, []
        
        logger.info(f"Processing {len(SCENARIOS)} scenarios with uncertainty-quantified GIC")
        
        for sc in SCENARIOS:
            logger.info(f"Processing scenario: {sc}")
            _, res, tbl = _simulate_scenario(sc, merged, config['theta0'], config['tolerance'], 
        ...
        if scen_res:
            ...
            if vuln_tbls:
                ...
                combined_vuln.to_parquet(DATA_LOC / OUTPUT_FILES['vulnerable_alpha_beta_uncertainty'])
            
            logger.info("Alpha-beta uncertainty scenario processing completed successfully")
            print("\nSummary Results:")
            print(summ_df)
        else:
        ...
        # Original batch processing logic for multiple files
        gic_files = []
        
        # Try each directory in the configuration
