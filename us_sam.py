...
from sam_balancer import balance_sam
...
def make_sam_accounts(use_tb_path, sup_tb_path):
    ...
    sam_accounts = list(industry_groups.keys()) + ['CAP', 'LAB', 'IDT', 'TARF', 'HOH', 'GOV', 'INV', 'EXT']
    ...
    for activity, codes in industry_groups.items():
        sam.at['TARF', activity] = supply_df.loc[codes, 'MDTY'].sum()
        
    # Taxes and tariffs to government
    sam.at['GOV', 'IDT'] = sam.loc['IDT'].sum()
    sam.at['GOV', 'TARF'] = sam.loc['TARF'].sum()
    ...
    print(f"Household savings: {total_savings} million (average of quarterly values)")
    ...
    print(f"Government deficit: {govt_deficit} million (based on average quarterly values)")
    ...
    print(f"Trade deficit: {trade_deficit} million (based on average quarterly values)")

    # Print key values for verification
    print("\nKey values for SAM (in millions):")
    print(f"1. Direct taxes (HOH→GOV): {direct_tax}")
    print(f"2. Government transfers (GOV→HOH): {transfers}")
    print(f"3. Household savings (HOH→INV): {total_savings}")
    print(f"4. Government deficit (GOV→INV): {govt_deficit}")
    print(f"5. Trade deficit (EXT→INV): {trade_deficit}")
    ...
    try:
        ...
        print(f"SAM update failed: {e}")

    # Remove subsidy in agric to part of govt payments to agric
    subsidy_amount = abs(sam.at['TARF', 'AGR'])
    sam.at['TARF', 'AGR'] = 0
    ...
    # Remove subsidy in agric to part of govt payments to agric
    subsidy_amount = abs(sam.at['TARF', 'G'])
    sam.at['TARF', 'G'] = 0
    ...
    try:
        ...
        print(f"Trade adjustment failed: {e}")
...
def balance_and_save_sam(sam, output_path="output/us_balanced_sam.csv", decimal_places=1):
    ...
    print(f"Balanced SAM saved to {output_path}")
...
if __name__ == "__main__":
    data_dir = Path("data")
    use_tb_path = data_dir / "use_tables.csv"
    sup_tb_path = data_dir / "supply_tables.csv"
    
    # Make sam
    sam = make_sam_accounts(use_tb_path, sup_tb_path)
    
    # Balance the SAM with 1 decimal place and save to CSV
    balanced_sam = balance_and_save_sam(sam, "data/us_balanced_sam.csv", decimal_places=1)
    
    # Show some information about the balanced SAM
    print("\nOriginal row sums:", sam.sum(axis=1).sum())
    print("Original column sums:", sam.sum(axis=0).sum())
    print("Balanced row sums:", balanced_sam.sum(axis=1).sum())
    print("Balanced column sums:", balanced_sam.sum(axis=0).sum())