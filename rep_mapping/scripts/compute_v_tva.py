...
from rep_mapping.rep_config import (
    TVA_DIR, HIFLD_PATH, GANNON_DS, CRS_GEO, setup_logger
)
...
def compute_voltages(tl_objects, tl_line_ids, E_pred, site_xys,
                     n_times, df_lines):
    ...
    name_to_pos   = {row["name"]: pos
                     for pos, (_, row) in enumerate(df_lines.iterrows())}
    ...
    logger.info(f"Lines with Gannon voltage: {(v_arr.any(axis=1)).sum()} / {len(df_lines)}")
