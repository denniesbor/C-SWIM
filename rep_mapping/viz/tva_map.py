...
def main():
    ...
    logger.info(
        f"Nodes with positions: {len(node_pos)} / {G_backbone.number_of_nodes()}"
    )
    ...
    if mon_xy:
        ...
        for n in mon_in_bb:
            if n not in node_pos:
                continue
            device = matched_devices[matched_devices["osmid"] == n]["device"].values[0]
            ax.annotate(
                device,
                xy=node_pos[n],
                fontsize=6,
    ...
    ax.set_title(
        "TVA Transmission Network Derived from OSM and HIFLD",
        fontsize=11,
        pad=8,
        loc="left",
    )
