...
from rep_mapping.rep_config import (
    HIFLD_PATH, OSM_SUB_PATH, TVA_BOUNDARY, DEVICES_NC,
    GRID_MAPPING, TVA_DIR, CRS_PROJ, CRS_GEO,
    VOLTAGE_THRESHOLD, SNAP_DISTANCE, KEEP_SYNTHETIC_ABOVE,
    LINE_RESISTANCE, TRAFO_WINDING_R, RG_REAL, RG_SYNTHETIC,
    POOL_GEN, POOL_TRANS, setup_logger
)
...
def build_endpoints(tva_lines):
    ...
    for idx, row in tva_lines.iterrows():
        ...
        records.append({"line_idx": idx, "end": "from",
                        "geometry": Point(coords[0]),  "VOLTAGE": row["VOLTAGE"]})
        records.append({"line_idx": idx, "end": "to",
                        "geometry": Point(coords[-1]), "VOLTAGE": row["VOLTAGE"]})
...
def match_endpoints_to_subs(ep_gdf, tva_subs_filt):
    ...
    logger.info(f"Matched: {ep_join['osmid'].notna().sum()}  "
...
def match_monitored_devices(tva_subs_filt, devices_gdf, ep_join):
    ...
    matched_devices  = gpd.sjoin(
        tva_subs_filt, devices_buf[["device", "type", "geometry"]],
        how="inner", predicate="intersects"
    )
    monitored_osmids  = set(matched_devices["osmid"].astype(float).unique())
    already_matched   = set(ep_join[ep_join["osmid"].notna()]["osmid"].astype(float).unique())
    missing_naturally = monitored_osmids - already_matched

    logger.info(f"Monitored devices: {len(monitored_osmids)}  "
...
def build_graph(tva_lines, ep_join):
    ...
    logger.info(f"Initial graph — nodes: {G.number_of_nodes()}  edges: {G.number_of_edges()}")
    return G
...
def prune_graph(G, min_voltage_keep=230, max_iterations=20):
    ...
    for i in range(max_iterations):
        dangles = [
            ...
            if d == 1
            and isinstance(n, str) and "_" in str(n)
            and G.get_edge_data(n, list(G.neighbors(n))[0]).get("voltage", 0) < min_voltage_keep
        ]
        if not dangles:
            break
...
def clean_graph(G, monitored_osmids):
    ...
    stranded = [n for n in monitored_osmids
                if n in G_pruned.nodes and G_pruned.degree(n) == 0]
    for n in stranded:
    ...
    singletons = [n for n, d in G_pruned.degree()
                  if d == 0 and float(n) not in monitored_osmids]
    G_pruned.remove_nodes_from(singletons)

    components = list(nx.connected_components(G_pruned))
    logger.info(f"Cleaned graph — nodes: {G_pruned.number_of_nodes()}  "
                f"edges: {G_pruned.number_of_edges()}  "
                f"components: {len(components)}  "
                f"largest: {max(len(c) for c in components)}")

    G_backbone = G_pruned.subgraph(
        max(nx.connected_components(G_pruned), key=len)).copy()

    in_backbone = monitored_osmids & set(G_backbone.nodes)
    logger.info(f"Monitored devices in backbone: {len(in_backbone)} / {len(monitored_osmids)}")
...
def build_substation_buses(G_backbone, tva_subs_filt, ep_gdf,
                            tva_lines, grid_mapping_df, rng_seed=42):
    ...
    osm_role   = trafo_rows.groupby("SS_ID")["Role"].agg(
    ...
    for node in G_backbone.nodes:
        ...
        if is_synthetic:
            ...
            lat, lon = (ep_match.iloc[0].geometry.y,
                        ep_match.iloc[0].geometry.x) if len(ep_match) else (35.5, -86.5)
        else:
            try:
        ...
        substation_buses[hv_bus] = {
            "SS_ID": hv_bus, "hv_bus": hv_bus, "lv_bus": lv_bus,
            "buses": [hv_bus, lv_bus], "HV_voltage": max_kv, "LV_voltage": 0,
            "Transformer_type": trafo_types[0], "trafo_types": trafo_types,
            "trafo_count": trafo_count, "is_synthetic": is_synthetic,
            "external_bus_to_hv_bus": [], "external_bus_to_lv_bus": [],
        }
        substations_rows.append({
            "name": hv_bus, "latitude": lat, "longitude": lon,
            "grounding_resistance": rg, "buses": [hv_bus, lv_bus], "kv_max": max_kv,
        })
    ...
    for u, v, d in G_backbone.edges(data=True):
        ...
        line_rows.append({
            "name": f"{u}_{v}", "from_bus": str(u), "to_bus": str(v),
            "V": voltage, "length": length_m / 1000.0, "R": R,
        })
    ...
    for info in substation_buses.values():
        for bus in sorted(info["buses"]):
            sub_look_up[bus] = idx; idx += 1
    for hv_bus in substation_buses:
        sub_look_up[hv_bus + "_neutral"] = idx; idx += 1

    logger.info(f"Substations: {len(substation_buses)}  Lines: {len(df_lines)}  "
                f"Matrix: {idx}x{idx}")
...
def main():
    ...
    substation_buses, bus_ids_map, sub_look_up, df_lines, substations_df = \
        build_substation_buses(G_backbone, tva_subs_filt, ep_gdf,
                               tva_lines, grid_mapping_df)
