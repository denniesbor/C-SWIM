...
def absmax_per_device(ds, source):
    ...
    gic_vars = [
        ...
        if {"time", "device"}.issubset(v.dims) and np.issubdtype(v.dtype, np.number)
    ]
    vname = (
        "gic"
        if "gic" in ds.data_vars
        else ("GIC" if "GIC" in ds.data_vars else gic_vars[0])
    )
    ...
    for key in [
        ...
        "_installation_type",
        "_connection",
        "_minimum_value_in_measurement_range",
    ]:
        if key in ds:
...
def load_gic_data(gannon_file, scaled_file, nc_path, scenarios):
    ...
    for yr in scenarios:
        ...
        sca_gdf = gpd.read_file(scaled_file)[[sub_key, "geometry", col_name]].rename(
            columns={col_name: f"scaled_{yr}yr"}
        )
    ...
    sim_gan = sim_gan.loc[np.abs(sim_gan["sim_gannon"]) <= 225].dropna(
        subset=["sim_gannon"]
    )
    ...
    for yr in scenarios:
        ...
        sim_data[yr] = sim_df.loc[np.abs(sim_df[f"sim_{yr}yr"]) <= 225].dropna(
            subset=[f"sim_{yr}yr"]
        )
...
def make_grid(gdf_list, cell):
    ...
    grid["region"] = grid.apply(
        lambda r: f"cell_{int(r.cell_i)}_{int(r.cell_j)}", axis=1
    )
    return grid
...
def process_regional_comparison(gan_gdf, scaled_data, sim_data, scenarios, cell_size_m):
    ...
    def regional_agg(tagged_gdf, lhs_col, rhs_col):
        ...
        return (
            ...
            .groupby("region")
            .agg(
                lhs_mean=("lhs_abs", "mean"),
                rhs_mean=("rhs_abs", "mean"),
                n=(sub_key, "size"),
            )
            .reset_index()
    ...
    gan_tag = gpd.sjoin(
        gan_gdf, grid[["region", "geometry"]], how="inner", predicate="within"
    )
    gan_tag = gan_tag.merge(
        sim_data["gannon"][[sub_key, "sim_gannon"]], on=sub_key, how="left"
    )
    ...
    for yr in scenarios:
        ...
        sca_tag = sca_tag.merge(
            sim_data[yr][[sub_key, f"sim_{yr}yr"]], on=sub_key, how="left"
        )
...
def plot_regression_validation(regional_results, xm, ym, cell_size_m, figures_dir):
    ...
    for i, (ax, (x, y), title) in enumerate(zip(axes, pairs, titles)):
        ...
        ax.scatter(
            x, y, s=48, alpha=0.55, facecolors="C0", edgecolors="#333", linewidths=0.7
        )
    ...
    for ext in ["png", "pdf"]:
        fig.savefig(
            figures_dir / f"regression_validation.{ext}", dpi=300, bbox_inches="tight"
        )
    plt.close(fig)
...
def main():
    ...
    gan_gdf, scaled_data, sim_data = load_gic_data(
        GANNON_INPUT_FILE, SCALED_INPUT_FILE, NC_PATH, SCENARIOS
    )

    logger.info("Computing regional comparisons...")
    regional_results, grid = process_regional_comparison(
        gan_gdf, scaled_data, sim_data, SCENARIOS, CELL_SIZE_M
    )
    ...
    meas_tag = gpd.sjoin(
        meas_gdf[["device", "gic_absmax", "geometry"]],
        grid[["region", "geometry"]],
        how="inner",
        predicate="within",
    )
    meas_regional = (
        meas_tag.groupby("region")
        .agg(meas_absmax_mean=("gic_absmax", "mean"))
        .reset_index()
    )
    reg_merge = regional_results["gannon"].merge(
        meas_regional, on="region", how="inner"
    )
