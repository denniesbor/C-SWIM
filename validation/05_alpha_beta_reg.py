...
class GICPredictor:
    ...
    def predict_gic(self, site_name, lat, lon, model_rank=0):
        ...
        if model_rank >= len(self.models):
            raise ValueError(f"Model rank {model_rank} exceeds available models ({len(self.models)})")
        ...
        for input_name in model["inputs"]:
            if input_name == "alpha":
                features.append(alpha);             feature_names.append("alpha")
            elif input_name == "interpolated_beta":
                features.append(beta);              feature_names.append("beta")
            elif input_name == "alpha*interpolated_beta":
                features.append(alpha * beta);      feature_names.append("alpha*beta")
            elif input_name == "mag_lat":
                features.append(mag_lat);           feature_names.append("mag_lat")
            elif input_name == "mag_lat*interpolated_beta":
                features.append(mag_lat * beta);    feature_names.append("mag_lat*beta")
...
def process_model_parallel(trafo_gdf, predictor, model_idx, n_processes=10):
    ...
    with Pool(n_processes) as pool:
        results = list(tqdm(pool.imap(predict_for_substation, args), total=len(args), desc=model_name))
...
def bootstrap_uncertainty(predictions_dict, rmse_dict, n_bootstrap=500):
    ...
    logger.info(f"Bootstrap uncertainty: {n_bootstrap} iterations, {n_substations} substations")
    ...
    for sub_idx in tqdm(range(n_substations), desc="Bootstrap"):
        ...
        if all_samples:
            ...
            stats = {k: np.nan for k in ["mean_prediction", "std_uncertainty", "conf_68_lower", "conf_68_upper"]}
...
def generate_substation_gic_analysis(
    substation_file, config, output_file,
    cc_threshold=0.75, n_processes=10, n_bootstrap=500, models_file=None,
):
    ...
    logger.info(f"Using {len(good_models)} models with cc > {cc_threshold} (of {len(models)} total)")

    predictions_dict, rmse_dict = {}, {}
    for i, model_idx in enumerate(good_models):
        model_name, predictions, rmse_values = process_model_parallel(trafo_gdf, predictor, model_idx, n_processes)
    ...
    for col in [f"model_{i+1}" for i in range(len(good_models))]:
        logger.info(f"{col}: mean={trafo_gdf[col].mean():.1f} A, max={trafo_gdf[col].max():.1f} A")
