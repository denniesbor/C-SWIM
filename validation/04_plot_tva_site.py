...
def compute_mag_mt_pairs(ds_tva_mag, simulated_ds):
    ...
    for i, (mag_lat, mag_lon) in enumerate(zip(mag_lats, mag_lons)):
        distances = [
            haversine_dist(mag_lat, mag_lon, mt_lat, mt_lon)
            for mt_lat, mt_lon in zip(mt_lats, mt_lons)
        ]
...
def compute_bounding_box(
    filtered_subs, trafo_unique, ds_tva_mag, pairs, mt_lats, mt_lons, mt_names
):
    """Compute spatial bounding box covering all validation sites."""
    all_lats, all_lons = [], []

    for sub in filtered_subs:
        lat, lon = trafo_unique[trafo_unique.sub_id == sub][
            ["latitude", "longitude"]
        ].values[0]
        all_lats.append(lat)
        all_lons.append(lon)
...
def create_sites_map(df_lines_EHV, tva_data, ds_tva_mag, simulated_ds):
    ...
    filtered_subs = [
        tva_data["valid_substations"][i] for i in tva_data["selected_indices"]
    ]
    gic_monitors = [
        tva_data["valid_match_ids"][i] for i in tva_data["selected_indices"]
    ]
    ...
    for pair in pairs:
        logger.info(
            f"MAG {pair['mag']}: MT {pair['mt']}, Distance {pair['distance']:.2f} km"
        )
    ...
    logger.info(
        f"Bounding box: {min_lat:.3f}, {min_lon:.3f} to {max_lat:.3f}, {max_lon:.3f}"
    )
    ...
    voltage_values = sorted(
        [v for v in lines_within_bbox["VOLTAGE"].unique() if pd.notna(v)]
    )
    palette = [
        "#E69F00",
        "#56B4E9",
    ...
    voltage_colors = {
        v: palette[i % len(palette)] for i, v in enumerate(voltage_values)
    }
    ...
    ax_us.add_feature(
        cfeature.STATES, linewidth=0.6, edgecolor="#666666", facecolor="none"
    )
    ...
    ax_tva.add_feature(
        cfeature.STATES, linewidth=0.8, edgecolor="#BBBBBB", facecolor="none"
    )
    ...
    for i, sub in enumerate(filtered_subs):
        sub_lat, sub_lon = (
            trafo_unique[trafo_unique.sub_id == sub][["latitude", "longitude"]]
            .values[0]
            .tolist()
        )
        gic_monitor = (
            gic_monitors[i][0] if isinstance(gic_monitors[i], list) else gic_monitors[i]
        )
        ax_tva.scatter(
            sub_lon,
            sub_lat,
        ...
        ax_tva.annotate(
            gic_monitor[0],
            xy=(sub_lon, sub_lat),
            xytext=(sub_lon + 0.15, sub_lat + 0.15),
            xycoords=temp_transform,
            color="black",
            fontsize=10,
        )
    ...
    for pair in pairs:
        ...
        mt_idx = np.where(mt_names == pair["mt"])[0][0]
        mag_lat = float(
            np.atleast_1d(ds_tva_mag.sel(device=mag_name).latitude.values)[0]
        )
        mag_lon = float(
            np.atleast_1d(ds_tva_mag.sel(device=mag_name).longitude.values)[0]
        )
    ...
    for lon, lat in [(x0, y0), (x1, y0), (x0, y1), (x1, y1)]:
        con = ConnectionPatch(
            xyA=(lon, lat),
            xyB=(lon, lat),
            coordsA=ax_us.transData,
            coordsB=ax_tva.transData,
            axesA=ax_us,
            axesB=ax_tva,
            color="red",
            lw=1.2,
    ...
    ax_tva.patch.set_path_effects(
        [pe.SimplePatchShadow(offset=(5, -5), alpha=0.28, rho=0.9), pe.Normal()]
    )
    for sp in ax_tva.spines.values():
        sp.set_zorder(11)

    transmission_elements = [
        Line2D([], [], color=voltage_colors[v], linewidth=2, label=f"{v} kV")
        for v in voltage_values
    ]
    site_elements = [
        Line2D(
    ...
    ax_leg.legend(
        handles=transmission_elements + site_elements,
        loc="center",
        frameon=False,
        fontsize=10,
    )
