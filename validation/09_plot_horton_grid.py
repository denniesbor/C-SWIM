...
def prepare_data(mc_results):
    ...
    mc_long = mc_grounds.melt(
        ...
        value_vars=cols, var_name="component", value_name="GIC",
    )
    ...
    for df in [mc_long, base_long]:
        df["Substation"] = pd.Categorical(df["Substation"], categories=SUBS_ORDER, ordered=True)
...
def plot_violin(mc_long, base_long, figures_dir):
    ...
    for i, (ax, comp) in enumerate(zip(axes, COMP_MAP.values())):
        ...
        sns.violinplot(
            data=d_mc, x="Substation", y="GIC", order=SUBS_ORDER,
            cut=0, inner=None, bw="scott", ax=ax,
            linewidth=0, color="#5fa8d3", saturation=1.0, width=0.9, scale="width",
        )
        ...
        sns.boxplot(
            data=d_mc, x="Substation", y="GIC", order=SUBS_ORDER, ax=ax,
            width=0.28, showcaps=False,
            ...
            flierprops=dict(marker=".", markersize=2, markeredgewidth=0, alpha=0.25, markerfacecolor="navy"),
            zorder=4,
        )

        base_vals = d_b.set_index("Substation").loc[SUBS_ORDER, "GIC"].values
        ax.scatter(
            x=np.arange(len(SUBS_ORDER)), y=base_vals,
            marker="o", s=34, linewidths=1.3,
            facecolors="white", edgecolors="#d00000", zorder=6, label="Baseline",
        )
    ...
    handles.append(Line2D([0], [0], color="#d00000", linewidth=1.4, label="Simulation median"))
    labels.append("Simulation median")
    axes[0].legend(handles, labels, frameon=False, loc="upper right", fontsize=8)

    plt.tight_layout(h_pad=0.5)
    for ext in ["png", "pdf"]:
        fig.savefig(figures_dir / f"horton_ground_gic_violin.{ext}", dpi=300, bbox_inches="tight")
    plt.close(fig)
