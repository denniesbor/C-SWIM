...
OUTPUT_FILE  = DATA_DIR / "regression" / "substations_with_gic_uncertainty_scaled.geojson"
H5_PATH      = DATA_DIR / "statistical_analysis" / "geomagnetic_data_return_periods.h5"

UNCERTAINTY_COLS = ["mean_prediction", "std_uncertainty", "conf_68_lower", "conf_68_upper"]
...
def load_return_period_data(h5_path):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        e_fields  = {period: f[f"predictions/E/{period}_year"][:] for period in RETURN_PERIODS}
    logger.info(f"Loaded {len(mt_coords)} MT coordinates and {len(RETURN_PERIODS)} return periods")
    return mt_coords, gannon_e, e_fields


def calculate_e_magnitude(e_data):
    """Calculate E-field magnitude from one or two components."""
    return e_data if e_data.ndim == 1 else np.sqrt(e_data[:, 0] ** 2 + e_data[:, 1] ** 2)
...
def add_e_field_columns(trafo_gdf, mt_coords, gannon_e, e_fields):
    ...
    trafo_gdf, gannon_interp = interpolate_to_substations(trafo_gdf, mt_coords, gannon_mag)
...
def scale_gic_uncertainty_to_return_periods(trafo_gdf, uncertainty_cols, return_periods):
    """Scale GIC uncertainty predictions by E-field ratio for each return period."""
    logger.info(f"Scaling {len(uncertainty_cols)} uncertainty columns to {len(return_periods)} return periods")
    for col in uncertainty_cols:
        for rp in return_periods:
            scaling_factor         = (trafo_gdf[f"e_{rp}yr_mag"] / trafo_gdf["e_gannon_mag"]).fillna(1.0)
...
def main():
    ...
    trafo_gdf = scale_gic_uncertainty_to_return_periods(trafo_gdf, UNCERTAINTY_COLS, RETURN_PERIODS)
