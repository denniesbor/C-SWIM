...
for beta, c in zip(beta_ln_range, colors_beta):
    mu = np.log(theta_fix)
    pdf = (1 / (beta * x_gic * np.sqrt(2 * np.pi))) * np.exp(
        -0.5 * ((np.log(x_gic) - mu) / beta) ** 2
    )
...
ax.text(
    0.0,
    y_loc,
    f"Effect of dispersion $\\beta$, $\\theta$ = {theta_fix} A/ph",
    fontsize=10,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.grid(True, axis="y", linestyle=":", linewidth=0.6, alpha=0.85)
ax.legend(
    loc="lower right", frameon=False, handlelength=1.5, bbox_to_anchor=(1.05, 0.0)
)
ax.text(
    0.0,
    y_loc + dy,
    "(b) GIC Failure Probability",
    fontsize=11,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.text(
    0.0,
    y_loc,
    f"Effect of threshold $\\theta$, $\\beta$ = {beta_fix}",
    fontsize=10,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
for beta_age, c in zip(beta_age_vals, colors_age):
    ...
    ax.plot(
        x_age, theta_age, color=c, linewidth=1.2, label=f"$\\beta_{{age}}$ = {beta_age}"
    )
...
ax.grid(True, axis="y", linestyle=":", linewidth=0.6, alpha=0.85)
ax.legend(
    loc="upper right", frameon=False, bbox_to_anchor=(1.05, 0.75), handlelength=1.5
)
ax.text(
    0.0,
    y_loc + dy,
    "(c) Age-Dependent Degradation",
    fontsize=11,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.text(
    0.0,
    y_loc,
    f"$\\theta(t)=\\theta_0(1-0.6F(t))$, $\\eta$ = {eta_age}y",
    fontsize=10,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.grid(True, axis="y", linestyle=":", linewidth=0.6, alpha=0.85)
ax.legend(
    loc="upper right", frameon=False, bbox_to_anchor=(1.05, 0.98), handlelength=1.2
)
ax.text(
    0.0,
    y_loc + dy,
    "(d) Weibull Age Distribution",
    fontsize=11,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.text(
    0.0,
    y_loc,
    f"Effect of shape $\\beta$, $\\eta$ = {eta_fix}y",
    fontsize=10,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.grid(True, axis="y", linestyle=":", linewidth=0.6, alpha=0.85)
ax.legend(
    loc="upper left", frameon=False, bbox_to_anchor=(0.05, 0.98), handlelength=1.5
)
ax.text(
    0.0,
    y_loc + dy,
    "(e) Age-Dependent Failure Rate",
    fontsize=11,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
...
ax.text(
    0.0,
    y_loc,
    f"Age-dependent fragility with dispersion effect",
    fontsize=10,
    transform=ax.transAxes,
    ha="left",
    va="bottom",
