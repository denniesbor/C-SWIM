...
def compare_electricity_failure_impacts(
    ...
    """
    logger.info(f"Analyzing economic impacts of electricity failure on {len(affected_sectors)} sectors")
...
def run_io_analysis(affected_sectors: Dict[str, float], sam_type: str = "us"):
    ...
    for sector, impact_pct in affected_sectors.items():
        if sector in d_va.index:
            ...
            logger.info(f"Applied {impact_pct*100:.1f}% reduction to {sector} value-added → shock of {shock_value:.2f}")
    ...
    logger.info(f"IO Model Results - Output changes by sector: {dx_va.round(2).to_dict()}")
    ...
    significant_contributions = {sector: sector_contributions[sector] 
...
def run_policy(model_instance, policy_params, scenario_name="Policy Simulation"):
    ...
    gov_LHS = sum(value(policy_model.pq[i]*policy_model.Xg[i]) for i in policy_model.i) \
              + value(policy_model.Tr) + value(policy_model.Sg)
    gov_RHS = value(policy_model.Td) \
    ...
    for h in policy_model.h:
        gap = sum(value(policy_model.F[h,i]) for i in policy_model.i) - value(policy_model.FF[h])
        print(f"Factor {h} market gap:", gap)

    bop_gap = sum(value(policy_model.pWe[i]*policy_model.E[i]) for i in policy_model.i) \
              + value(policy_model.Sf) \
              - sum(value(policy_model.pWm[i]*policy_model.M[i]) for i in policy_model.i)
    print("BoP gap:", bop_gap)
    
    baseline_gdp = sum(value(model_instance.py[i]) * value(model_instance.Y[i]) for i in model_instance.i)
    policy_gdp   = sum(value(policy_model.py[i])   * value(policy_model.Y[i])   for i in policy_model.i)
...
def compare_model_results(io_results, cge_results):
    ...
    logger.info(f"Sectoral impacts comparison (IO vs CGE): {comparison.round(2).to_dict()}")
    
    logger.info(f"IO Total Output Impact: {io_total:.2f}")
    logger.info(f"CGE Total Output Impact: {cge_total:.2f}")
    logger.info(f"Difference: {cge_total - io_total:.2f} ({(cge_total - io_total)/io_total*100:.2f}%)")
