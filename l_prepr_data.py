...
from configs import (
    ...
    DENNIES_DATA_LOC, ALPHA_BETA_SCENARIOS, REGULAR_SCENARIOS
)
...
def load_and_aggregate_tiles():
    ...
    econ_cols = [c for c in merged.columns
...
def load_gic_results():
    ...
    if USE_ALPHA_BETA_SCENARIO:
        combined_ds = xr.open_dataset(DATA_LOC / "scenario_summary_alpha_beta_uncertainty.nc")
        combined_vuln = pd.read_parquet(DATA_LOC / "vulnerable_substations_alpha_beta_uncertainty.parquet")
...
def process_vulnerability_chunks(combined_vuln, chunk_size=50, max_realizations=2000):
    ...
    for scenario_name in combined_vuln['scenario'].unique():
        ...
        scenario_data['gic_realization'] = pd.to_numeric(scenario_data['gic_realization'])
        
        all_realizations = sorted(scenario_data['gic_realization'].unique())
        max_real = min(len(all_realizations), max_realizations)
        
        for start_idx in tqdm(range(0, max_real, chunk_size), desc=f"Chunks for {scenario_name}"):
            ...
            chunk_grouped = chunk_data.groupby('sub_id')['failure_prob'].agg(['sum', 'count'])
...
def find_vulnerable_substations(mean_vuln_data, years_of_interest=[100, 150, 200, 250], threshold=0.5):
    """Find substations vulnerable across multiple scenarios"""
    if USE_ALPHA_BETA_SCENARIO:
        target_scenarios = [sc for sc in ALPHA_BETA_SCENARIOS if any(str(y) in sc for y in years_of_interest)]
    else:
        target_scenarios = [sc for sc in REGULAR_SCENARIOS if any(str(y) in sc for y in years_of_interest)]
