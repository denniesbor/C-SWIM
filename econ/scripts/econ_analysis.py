        ...
        for rp in return_periods:
            ...
            if mean_data is not None:
                gdp_vars = [
                    ...
                    if var.startswith("GDP_") and var.endswith("_affected")
                ]
                ...
                est_vars = [
                    var
                    for var in ds.data_vars
                    if var.startswith("EST_") and var.endswith("_affected")
                ]
                est_mean = sum(float(mean_data[var].values) for var in est_vars)
                est_lower = (
                    sum(float(lower_data[var].values) for var in est_vars)
                    if lower_data is not None
                    else est_mean
                )
                est_upper = (
                    sum(float(upper_data[var].values) for var in est_vars)
                    if upper_data is not None
                    else est_mean
                )
    ...
    for scenario in confidence_df["scenario"].unique():
        ...
        for conf_level in ["mean", "p5", "p95"]:
            ...
            for sector in io.sectors:
                results.append(
                    {
                        "scenario": scenario.replace("e_", "").replace(
                            "-hazard A/ph", ""
                        ),
...
def main():
    ...
    if USE_ALPHA_BETA_SCENARIO:
        logger.info(f"Saving Results -- using alpha beta scenarios")
        io_results_df.to_csv(
            FIGURES_DIR / "io_model_results_alpha_beta.csv", index=False
        )

        confidence_df.to_csv(
            FIGURES_DIR / "confidence_intervals_alpha_beta.csv", index=False
        )

    elif PROCESS_GND_FILES:
        logger.info(f"Saving Results -- with gnd gic")
        io_results_df.to_csv(FIGURES_DIR / "io_model_results_gnd_gic.csv", index=False)

        confidence_df.to_csv(
            FIGURES_DIR / "confidence_intervals_gnd_gic.csv", index=False
        )

    else:
        logger.info(f"Saving Results -- with gic sim")
