...
def load_gic_results():
    ...
    batch_dir = (
        DATA_LOC / "gic_processing_batches"
        if not PROCESS_GND_FILES
        else DATA_LOC / "gnd_gic_processing_batches"
    )
...
def load_and_process_gic_data(df_lines):
    ...
    v_cols = ["V_halloween", "V_st_patricks", "V_gannon", "V_hydro_quebec"] + [
    ...
    df_lines.loc[mask, "V_hydro_quebec"] = hydro_quebec_v[indices[mask]]
